...
export class ClientGrpcController {
  ...
  async checkOrCreate(
    ...
    const { platform, platformId, name } = input.data;

    console.log('ClientGrpcController đã nhận được yêu cầu!');

    const ok = await this.clientUseCase.checkOrCreateClient({ platformId, platform: platform as PlatformEnum, name });
