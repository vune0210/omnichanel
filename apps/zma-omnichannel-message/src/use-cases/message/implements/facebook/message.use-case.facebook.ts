...
@Injectable()
export class MessageUseCaseFacebook implements IMessageInterface {
  constructor(
  ...
  async addAgentMessage(input: MessageInput): Promise<boolean> {
    ...
    const tenantId = 'your_tenant_id_here';
    const pageAccessToken = this.configService.get<string>('NX_FACEBOOK_PAGE_ACCESS_TOKEN');
    const responseUrl = `https://graph.facebook.com/v23.0/me/messages?access_token=${pageAccessToken}`;
    ...
    try {
      ...
      console.error('❌ Gửi tin nhắn tới Facebook thất bại:', err.response?.data || err.message);
      return false;
    }
  }
  ...
  // }) {
  //   const tenantId = 'your_tenant_id_here';
  //   await this.dataServices.messageService.markMessageAsDelivered({
  //     tenantId,
  //     clientPlatformId,
  //     platform: PlatformEnum.Facebook,
  //     watermark,
  ...
  //   });
  //   const topic = `chat/${PlatformEnum.Facebook}/${clientPlatformId}/statusUpdate`;
  //   console.log('Publishing MQTT message to topic:', topic);
  ...
  // }) {
  //   const tenantId = 'your_tenant_id_here';
  //   await this.dataServices.messageService.markMessageAsRead({
  //     tenantId,
  //     clientPlatformId,
  //     platform: PlatformEnum.Facebook,
  //     watermark,
