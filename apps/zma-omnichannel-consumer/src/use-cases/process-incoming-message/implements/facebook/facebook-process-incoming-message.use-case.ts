...
@Injectable()
export class FacebookProcessIncomingMessageUseCase
  implements ProcessIncomingMessageUseCase, OnModuleInit
{
  private readonly logger = new Logger(FacebookProcessIncomingMessageUseCase.name);
  ...
  private async checkAndCacheUser(platformId: string): Promise<void> {
    const cacheKey = `user:${Platform.Facebook}:${platformId}`;
    await this.cacheManager.wrap(cacheKey, async () => {
      try {
        const wrapper = new MicroserviceInput<ClientServiceCheckOrCreateInput>({
          ...
          data: { platform: Platform.Facebook, platformId: platformId },
        });
  ...
  private async handleUserMessage(
    ...
    await this.daprProducer.sendMessage(KafkaTopicsEnum.FacebookMessageReceived, payload);
    this.logger.log('📩 Sent to FACEBOOK_MESSAGE_RECEIVED');
    //this.logger.log('📩 Payload: ' + JSON.stringify(payload, null, 2));
  }

  private async handleDelivery(timestamp: number, platformId: string): Promise<void> {
    await this.daprProducer.sendMessage(KafkaTopicsEnum.FacebookMessageDelivered, {
      platform: Platform.Facebook,
      platformId,
      watermark: timestamp,
    });
    this.logger.log('📦 Sent FACEBOOK DELIVERY event');
  }

  private async handleRead(timestamp: number, platformId: string): Promise<void> {
    await this.daprProducer.sendMessage(KafkaTopicsEnum.FacebookMessageRead, {
      platform: Platform.Facebook,
      platformId,
      watermark: timestamp,
    });
    this.logger.log('📖 Sent FACEBOOK READ event');
  }
}
